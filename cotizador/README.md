...
.venv-pricing/bin/python cotizador/extract.py "lista de precios nueva.pdf"
.venv-pricing/bin/python cotizador/tests/verify.py "lista de precios nueva.pdf"
```

Output is written to `cotizador/data/`, which is **wiped first** so a family
...
Open **`cotizador/index.html`** directly in a browser (double-click — no
